...
A GPigT hunts the nearest sign that holds a player's question, answers it, and
must never re-answer the same sign or treat another GPigT's answer as a new
question. Multiple GPigTs may operate in the same world. We therefore need each
...
- distinguishes a player's question from a GPigT's answer from an unrelated
  decorative sign,
- can be reset when a player edits an answered sign to continue the thread.
...
- `SignState { NONE, QUESTION, CLAIMED, ANSWER }` (see CONTEXT.md for the
...
- A **Mixin** on the server-side sign-edit path
  (`ServerGamePacketListenerImpl#updateSignText`) sets a sign to `QUESTION` when
  a player writes non-blank text. This is what moves a sign `NONE → QUESTION`
  and resets `ANSWER → QUESTION` (and `CLAIMED → QUESTION`) for thread
  continuation.
- When a GPigT reaches a `QUESTION` sign to ponder it, it sets the sign to
  ...
  `CLAIMED` signs, so two GPigTs never answer the same question.
- When a GPigT writes its response it sets the sign to `ANSWER`. This single
  write is what stops re-answering and stops other GPigTs from treating the
  answer as a question — no per-pig bookkeeping required.
- Claim cleanup is **timestamp-based**: any GPigT that finds a `CLAIMED` sign
  whose `CLAIM_TICK` is older than a threshold treats it as `QUESTION` and
...
- Multi-GPigT correctness falls out of the shared flag: any GPigT respects
  `ANSWER` regardless of which GPigT wrote it.
...
- We take on one Mixin into a vanilla packet handler, because Fabric exposes no
  ...
  is never misread as a player question. It must also ignore blank edits, so
  closing the edit GUI on an empty sign does not create a phantom `QUESTION`.
...
- **A shared set of answered `BlockPos` in world save data** — rejected. The
